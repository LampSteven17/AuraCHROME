...
func TestWizardFlowHeadless(t *testing.T) {
	...
	if !strings.Contains(m.inOutput.Value(), "aurachrome_tif") {
	...
	// exercise option controls
	m = advance(t, m, key("right")) // change look
	if m.lookIdx != 1 {
	...
	for _, want := range []string{"-i " + dir, "--preset punchy", "--no-grain", "--cpu", "--jobs 5", "--progress-json"} {
	...
	m.step = stepRun
	m = advance(t, m, engStartMsg{total: 3, device: "gpu", outdir: "/out"})
